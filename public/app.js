...
function comparePronunciation(expected, spoken) {
  ...
  const similarity = maxLen ? Math.max(0, Math.round((1 - dist / maxLen) * 100)) : 0;
...
if (recognition) {
  recognition.onstart = () => {
    isListening = true;
    statusText.textContent = "Ouvindo... fale em inglês.";
  };
  ...
  recognition.onend = () => {
    isListening = false;
    if (!statusText.textContent.startsWith("Erro") && !statusText.textContent.startsWith("Resposta")) {
...
renderHistory();
expectedPhrase.textContent = "A frase corrigida aparecerá aqui para você repetir.";
spokenPhrase.textContent = "...";
similarityScore.textContent = "0%";
wordScore.textContent = "0";
pronunciationFeedback.textContent = "Fale e compare sua frase com a correção sugerida.";
